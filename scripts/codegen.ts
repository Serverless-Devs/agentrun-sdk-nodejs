...
function generateControlApiCode(config: Config, source: string): string {
  ...
  for (const method of config.methods) {
    ...
    const { typeString: paramsType, paramNames } = generateParamsType(
      method,
      sdkPrefix,
    );
    ...
    for (const param of method.params) {
      lines.push(
        `   * @param params.${toCamelCase(param.name)} - ${param.description}`,
      );
    }
    ...
    for (const param of method.params) {
      ...
      if (param.wrapperType) {
        callParams.push(
          `new ${sdkPrefix}.${param.wrapperType}({ body: ${paramName} })`,
        );
      } else if (
        param.optional &&
        param.type.charAt(0) === param.type.charAt(0).toUpperCase()
      ) {
    ...
    lines.push(
      `      const response = await client.${apiMethodName}WithOptions(`,
    );
 ...
 */
function main(): void {
  console.log("🚀 AgentRun SDK Code Generation");
  console.log("================================");
  ...
  if (!fs.existsSync(configsDir)) {
    console.log("❌ No configs directory found, skipping codegen");
    return;
  }
  ...
  if (configFiles.length === 0) {
    console.log("❌ No YAML config files found");
    return;
  }
