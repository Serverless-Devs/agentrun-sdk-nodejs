...
class StreamState {
  ...
  *endTextIfOpen(): Generator<Record<string, unknown>> {
    if (this.text.started && !this.text.ended) {
      yield { type: AGUI_EVENT_TYPES.TEXT_MESSAGE_END, messageId: this.text.messageId };
...
export class AGUIProtocolHandler extends ProtocolHandler {
  ...
  private *processEvent(
    ...
    if (event.event === EventType.HITL) {
      ...
      const toolCallId = (event.data?.tool_call_id as string) || (event.data?.toolCallId as string) || '';
    ...
    if (event.event === EventType.TOOL_RESULT) {
      ...
      yield {
        ...
        messageId: (event.data?.message_id as string) || (event.data?.messageId as string) || `tool-result-${toolId}`,
        toolCallId: toolId,
        content: finalResult,
        role: 'tool',
      };
    ...
    if (event.event === EventType.STATE) {
      if ('snapshot' in (event.data || {})) {
        yield { type: AGUI_EVENT_TYPES.STATE_SNAPSHOT, snapshot: event.data?.snapshot || {} };
      } else if ('delta' in (event.data || {})) {
        yield { type: AGUI_EVENT_TYPES.STATE_DELTA, delta: event.data?.delta || [] };
      } else {
        yield { type: AGUI_EVENT_TYPES.STATE_SNAPSHOT, snapshot: event.data || {} };
      }
      return;
    }
  ...
  private async *errorStream(message: string): AsyncGenerator<string> {
    ...
    yield this.encode({ type: AGUI_EVENT_TYPES.RUN_ERROR, message, code: 'REQUEST_ERROR' });
  }
}
