...
import { AgentInvoker, InvokeAgentHandler } from './core/invoker';
...
export class AgentRunServer {
    ...
    this.server.listen(port, host, () => {
      ...
      for (const protocol of this.protocols) {
        ...
        for (const route of routes) {
          logger.info(`  ${route.method} ${protocol.getPrefix()}${route.path} - ${protocol.name} protocol`);
        }
      }
   ...
   */
  private toProtocolRequest(req: http.IncomingMessage, body: Record<string, unknown>): ProtocolRequest {
