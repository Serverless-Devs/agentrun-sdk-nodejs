...
export class BrowserToolSet extends SandboxToolSet {
  ...
  private async loadPlaywright(): Promise<typeof import('playwright')> {
    try {
      ...
      logger.error('Playwright is not installed. Please install it with: npm install playwright');
      logger.debug(`${error}`);
      throw new Error(
 ...
 * Requires optional 'playwright' peer dependency for full functionality.
 */
export class AIOToolSet extends SandboxToolSet {
  private playwrightAIO: Browser | null = null;
  private currentPage: Page | null = null;
  private pages: Page[] = [];
  ...
  private async ensurePlaywright(): Promise<{
    ...
    // Connect Playwright if not connected
    if (!this.playwrightAIO) {
      const playwright = await this.loadPlaywright();
      const cdpUrl = browserSandbox.getCdpUrl();
      this.playwrightAIO = await playwright.chromium.connectOverCDP(cdpUrl);

      // Get existing contexts/pages or create new ones
      const contexts = this.playwrightAIO.contexts();
      if (contexts.length > 0) {
        const existingPages = contexts[0].pages();
        if (existingPages.length > 0) {
          this.pages = existingPages;
  ...
  override close() {
    if (this.playwrightAIO) {
      this.playwrightAIO.close().catch(e => {
        logger.debug('Failed to close Playwright browser:', e);
      });
      this.playwrightAIO = null;
      this.currentPage = null;
      this.pages = [];
    }
    super.close();
...
export async function sandboxToolset(
  ...
  if (templateType === TemplateType.BROWSER)
