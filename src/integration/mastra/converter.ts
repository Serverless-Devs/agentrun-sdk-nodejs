...
export class MastraConverter {
  ...
  *convert(chunk: MastraChunkBase): Generator<AgentEventItem> {
    ...
    logger.debug(
      `[MastraConverter] Unsupported chunk type: ${chunk.type}`,
    );
  }
   ...
   */
  private extractTextFromPayload(payload?: Record<string, unknown>): string | null {
   ...
   */
  private extractToolCallFromPayload(payload?: Record<string, unknown>): {
    id: string;
    name: string;
    args: string;
  } | null {
   ...
   */
  private extractToolResultFromPayload(payload?: Record<string, unknown>): {
    id: string;
    result: string;
  } | null {
    if (!payload) return null;
